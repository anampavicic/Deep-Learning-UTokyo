...
with open("configs/audio_model_modified_data_best.json", "r") as f:
...
for fold, (train_val_idx, val_idx) in enumerate(kf.split(df)):
    ...
    model_save_path = f'models/model_modified_fold_{fold + 1}.pth'
...
results_df.to_csv("results/kfold_modified_results.csv", index=False)
print("Saved k-fold results to results/kfold_modified_results.csv")

# Save best model info
print(f"Best model: {best_model_path} with accuracy: {best_accuracy:.2f}%")

best_model_target = "models/best_modified_model.pth"
...
# save the confusion matrix and graphs
max_val_loader = val_loader_list[best_model_index]
...
# === Plots ===
Utilities.plot_confusion_matrix_animals(y_true, y_pred_logits, dataset_val.class_to_idx)
Utilities.plot_graphs(max_train_acc_list, max_val_acc_list)
